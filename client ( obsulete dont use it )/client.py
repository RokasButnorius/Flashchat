...
class CallDialog(QDialog):
    ...
    def _start(self):
        ...
        asyncio.ensure_future(self.call_manager.start_call(self.peer_id, mic_device=mic, camera_device=cam))

    def _accept(self, offer_payload):
        self.status_label.setText("Connecting...")
        asyncio.ensure_future(self.call_manager.handle_offer(self.peer_id, offer_payload))
...
class LoginDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Privacy Messenger -- Sign In")
        self.setMinimumWidth(360)
        ...
        layout = QVBoxLayout(self)
        title = QLabel("🔒 Privacy Messenger")
...
class MainWindow(QMainWindow):
    def __init__(self, session: GuiSession):
        super().__init__()
        self.session = session
        self.setWindowTitle(f"Privacy Messenger -- {session.user_id}")
    ...
    def _handle_rtc_signal(self, kind, from_user, payload):
        if kind == "offer":
            ...
            dlg.exec()
        # answer/ice are routed to the active CallManager instance in a
        # fuller build via a signaling bus; kept minimal here since the
        # offer/answer/accept flow above covers the core call path.


async def run_login_flow(session_holder: dict):
    login = LoginDialog()
...
def main():
    ...
    session_holder = {}
    with loop:
        loop.run_until_complete(run_login_flow(session_holder))
