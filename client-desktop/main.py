...
class LoginDialog(QDialog):
    ...
    def _submit_login(self):
        """Explicit login: fails clearly if no local vault exists for this
        user ID on this device, instead of silently creating a brand new
        identity (the old single-button flow's actual bug -- typing an
        existing username you don't have a local vault for used to just
        register a second, unrelated identity under that name)."""
        ...
        if not is_anon:
            ...
            if not probe.vault.exists():
                QMessageBox.warning(
                    ...
                    "If you already have an account, make sure you're on the same "
                    "device/passphrase you signed up with. Otherwise, use Sign Up.")
                return
    ...
    def _submit_signup(self):
        """Explicit signup: refuses to silently overwrite an existing local
        vault under the same user ID."""
        if not self._validate_common():
            return
        user_id = self.user_input.text().strip()
        is_anon = self.anon_check.isChecked()
        if not is_anon:
            ...
            if probe.vault.exists():
                QMessageBox.warning(
                    ...
                    f"A local account for \"{user_id}\" already exists on this device.\n\n"
                    "Use Log In instead, or pick a different user ID.")
                return
        self.result_user_id = user_id
        self.result_passphrase = self.pass_input.text()
        self.result_anonymous = is_anon
...
async def do_connect(user_id: str, passphrase: str, is_anonymous: bool, is_signup: bool, session_holder: dict):
...
def main():
    ...
    with loop:
        try:
            loop.run_until_complete(do_connect(user_id, passphrase, is_anonymous, is_signup, session_holder))
        except Exception as e:
            import traceback
