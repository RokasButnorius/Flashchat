...
class ChatScreen(Screen):
    def __init__(self, app, **kwargs):
        ...
        self.call_btn = Button(text="\U0001F4DE", size_hint=(0.15, 1),
                               background_color=COL_INPUT, color=COL_TEXT, disabled=True)
    ...
    def on_incoming_call(self, from_user):
        """Simple accept/decline popup -- no separate call screen for the
        first pass, matches the minimal-viable-calling scope."""
        content = BoxLayout(orientation="vertical", spacing=10, padding=10)
        content.add_widget(Label(text=f"Incoming call from {from_user}", color=COL_TEXT))
        btn_row = BoxLayout(spacing=10, size_hint=(1, 0.4))
        popup = Popup(title="Incoming Call", content=content, size_hint=(0.8, 0.4), auto_dismiss=False)
        ...
        accept_btn = Button(text="Accept", background_color=COL_ACCENT)
        accept_btn.bind(on_release=accept)
        decline_btn = Button(text="Decline", background_color=COL_INPUT)
...
class FlashChatApp(App):
    ...
    def build(self):
        ...
        self._pending_offer = None  # (from_user, payload) while an incoming-call popup is showing
    ...
    def _hook_session_callbacks(self, session):
        ...
        self.call_manager = calling.CallManager(
            session, self.bridge, on_state_change=self._on_call_state, on_call_ended=self._on_call_ended)
    ...
    def _show_call_security_warning(self):
        ...
        content.add_widget(Label(
            ...
            color=COL_TEXT))
        popup = Popup(title="Call security warning", content=content, size_hint=(0.85, 0.5))
        close_btn = Button(text="OK", size_hint=(1, 0.3), background_color=COL_ACCENT)
        close_btn.bind(on_release=lambda *_: popup.dismiss())
        content.add_widget(close_btn)
        popup.open()

    def _on_call_state(self, state_str):
        Clock.schedule_once(lambda _dt: hasattr(self, "chat_screen") and self.chat_screen.on_call_state(state_str))

    def _on_call_ended(self):
        Clock.schedule_once(lambda _dt: hasattr(self, "chat_screen") and self.chat_screen.on_call_ended())
